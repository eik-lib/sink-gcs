	...
	write(filePath, contentType, options = {}) {
		return new Promise((resolve, reject) => {
			...
			gcsStream.on("error", () => {
				...
				// The raw 412 is left to propagate through the pipeline unchanged;
				// the caller (writeJSONWithOptions) reclassifies it using the options
				// that triggered the condition. Re-emitting here caused the error
				// handler to fire multiple times for a single event, accumulating
				// extra listeners on internal streams under concurrent load.
			});
