...
const SinkGCS = class SinkGCS extends Sink {
	...
	write(filePath, contentType, options = {}) {
		return new Promise((resolve, reject) => {
			...
			/** @type {import('@google-cloud/storage').CreateWriteStreamOptions} */
			...
			if (options.ifNotExists) {
				// ifGenerationMatch: 0 means "only write if the object does not exist".
				// GCS rejects with HTTP 412 Precondition Failed when it does.
				writeStreamOptions.preconditionOpts = { ifGenerationMatch: 0 };
			} else if (options.ifGenerationMatch !== undefined) {
				writeStreamOptions.preconditionOpts = {
					ifGenerationMatch: Number(options.ifGenerationMatch),
				};
			}

			const gcsStream = src.createWriteStream(writeStreamOptions);

			gcsStream.on("error", (/** @type {any} */ err) => {
				this._counter.inc({ labels: { access: true, operation } });
				// GCS returns HTTP 412 when a precondition (ifGenerationMatch) fails.
				if (err.code === 412) {
					const conflict = options.ifNotExists
						? Object.assign(new Error(`File already exists: ${filePath}`), {
								code: "ALREADY_EXISTS",
							})
						: Object.assign(new Error(`Generation mismatch for ${filePath}`), {
								code: "CONFLICT",
							});
					// Re-emit as a higher-level error so pipeline() receives it.
					gcsStream.emit("error", conflict);
				}
			});
		...
		return new Promise((resolve, reject) => {
			...
			gcsStream.on("response", (response) => {
				if (response.statusCode === 200) {
					...
					// Expose the GCS object generation so callers can perform
					// compare-and-swap writes via options.ifGenerationMatch.
					// @ts-ignore — ReadFile.generation in @eik/common >= next release
					obj.generation = response.headers["x-goog-generation"] ?? "";
	...
	delete(filePath) {
		return new Promise((resolve, reject) => {
			...
			// Attempt to delete the object at the exact path first. This covers
			// single-file paths (e.g. health check temp files) without requiring
			...
			src
				...
				.catch((err) => {
					...
					// No object at the exact path; fall back to prefix-based deletion
					// for package directories which store multiple GCS objects.
					this._bucket.deleteFiles(
						{ prefix: pathname, force: true },
						(error) => {
							if (error) {
								this._counter.inc({ labels: { access: true, operation } });
								return reject(error);
